...
//!
//! A manifest names every installed package file and the digest it had when somebody
//! acknowledged it; a file whose digest does not match does not run. Only code that arrived by
//! being fetched is held back — a file in your own `plugin/` directory is one you put there.
...
/// What the manifest says. A manifest that is missing or will not parse reads as empty, which
...
mod tests {
    ...
    fn a_file_that_changed_after_being_acknowledged_is_not_cleared_and_is_known() {
        let dir = Scratch::new("casper-ack", "changed");
    ...
    fn acknowledging_replaces_rather_than_accumulates() {
        let dir = Scratch::new("casper-ack", "replace");
        let manifest = manifest_in(&dir);
    ...
    fn a_manifest_that_will_not_parse_holds_everything_back() {
        let dir = Scratch::new("casper-ack", "broken");
        let manifest = manifest_in(&dir);
