//! JSON for the VM: a protocol description reads provider payloads, and every one of them is
//! JSON.

use crate::lua::convert::{json_from_lua, lua_from_json};
...
pub fn table<'gc>(ctx: Context<'gc>) -> Table<'gc> {
    ...
    let decode = Callback::from_fn(&ctx, |ctx, _exec, mut stack| {
        ...
        // A malformed payload yields nil rather than raising: one bad frame must not lose the turn.
